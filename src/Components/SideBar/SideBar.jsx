...
const SideBar = ({
  ...
  setMainCourseData,
  resetSelectedIds,
  semesterDropdownInfo,
  chapterDropdownInfo,
  setSemesterDropdownInfo,
  setChapterDropdownInfo,
  handleSlectedIds,
  setIsDeleted,
  isDataSaved,
  setShowSaveDataModal,
  selectedSemId,
  selectedChapterId,
  sidebarData,
  setSidebarData,
  ...
  const addSemester = () => {
    ...
    setError(false);
    axios
      .post("http://localhost:3001/api/add-semester", {
        name: semesterName,
        courseId: courseId,
      })
      .then((response) => {
  ...
  function addChapter(semesterId) {
    ...
    if (!mainCourseData) {
      axios
        .post("http://localhost:3001/api/add-chapter", {
          name: chapterName,
          courseId: courseId,
        ...
        .then((response) => {
          if (response.data.status === "success") {
            axios
              .get("http://localhost:3001/api/get-parent-data", {
                params: {
                  semesterId: semesterId,
                  courseId: courseId,
                },
  ...
  function addSection(semesterId, chapterId) {
    ...
    if (!mainCourseData) {
      axios
        .post("http://localhost:3001/api/add-section", {
          name: sectionName,
          courseId: courseId,
          semesterId: semesterId,
          chapterId: chapterId,
  ...
  function addSectionLevelQuiz(semesterId, chapterId) {
    ...
    if (!mainCourseData) {
      axios
        .post("http://localhost:3001/api/add-chapter-test", {
          name: quizName,
          chapterId: chapterId,
          content: {
            slides: [
              {
                id: uuidv4(),
                content: [{ id: uuidv4(), type: "Quiz", data: null }],
              },
            ],
          },
        })
  ...
  function addQuizBelowChapters(semesterId) {
    ...
    if (!mainCourseData) {
      axios
        .post("http://localhost:3001/api/add-semester-test", {
          name: quizName,
          semesterId: semesterId,
          content: {
            slides: [
  ...
  function deleteSemester(semId) {
    if (!mainCourseData) {
      ...
      axios
        .delete("http://localhost:3001/api/delete-semester", {
          data: {
            semesterId: semId,
          },
        })
        .then((response) => {
          if (response.data.status === "success") {
            setSidebarData((sidebarData) => {
              const newSemesters = sidebarData.semesters.filter(
                (semester) => semester.id !== semId
              );
              return { semesters: newSemesters };
            });
          }
  ...
  function editSemesterName(semId) {
    ...
    axios
      .put("http://localhost:3001/api/edit-semester-name", {
        semesterId: semId,
        name: newName,
      })
      .then((response) => {
  ...
  function deleteChapter(semId, chapId) {
    if (!mainCourseData) {
      axios
        .delete("http://localhost:3001/api/delete-chapter", {
          data: {
            chapterId: chapId,
          },
        })
            ...
            setSidebarData((sidebarData) => {
              const newSemesters = sidebarData.semesters.map((semester) => {
                if (semester.id === semId) {
                  return {
                    ...semester,
                    chapters: semester.chapters.filter(
                      (chapter) => chapter.id !== chapId
                    ),
                  };
                } else {
                  return semester;
  ...
  function deleteSection(semId, chapId, sectionId) {
    if (!mainCourseData) {
      ...
      axios
        .delete("http://localhost:3001/api/delete-section", {
          data: {
            sectionId: sectionId,
          },
        })
  ...
  function editChapterName(semId, chapId) {
    ...
    axios
      .put("http://localhost:3001/api/edit-chapter-name", {
        chapterId: chapId,
        name: newName,
      })
      .then((response) => {
  ...
  function editSectionName(semId, chapId, sectionId) {
    ...
    axios
      .put("http://localhost:3001/api/edit-section-name", {
        sectionId: sectionId,
        name: newName,
      })
      .then((response) => {
  ...
  function editChapterLevelQuizName(semId, quizId) {
    ...
    axios
      .put("http://localhost:3001/api/edit-semesterTest-name", {
          ...
          setSidebarData({
            semesters: sidebarData.semesters.map((semester) => {
              if (semester.id === semId) {
                return {
                  ...semester,
                  semesterTest: semester.semesterTest.map((q) => {
                    if (q.id === quizId) {
                      return {
                        ...q,
                        name: newName,
  ...
  function editQuizName(semId, chapId, quizId) {
    ...
    axios
      .put("http://localhost:3001/api/edit-chapterTest-name", {
                ...
                return {
                  ...
                  chapters: semester.chapters.map((chapter) => {
                    if (chapter.id === chapId) {
                      return {
                        ...chapter,
                        chapterTest: chapter.chapterTest.map((q) => {
                          if (q.id === quizId) {
                            return {
                              ...q,
                              name: newName,
  ...
  function deleteChapterLevelQuiz(semId, quizId) {
    if (!mainCourseData) {
      axios
        .delete("http://localhost:3001/api/delete-semesterTest", {
          data: {
            semesterTestId: quizId,
          },
        })
            ...
            setSidebarData((sidebarData) => {
              const newSemesters = sidebarData.semesters.map((semester) => {
                if (semester.id === semId) {
                  return {
                    ...semester,
                    semesterTest: semester.semesterTest.filter(
                      (semesterTest) => semesterTest.id !== quizId
                    ),
                  };
                } else {
                  return semester;
  ...
  function deleteSectionLevelQuiz(semId, chapId, quizId) {
    if (!mainCourseData) {
      axios
        .delete("http://localhost:3001/api/delete-chapterTest", {
          data: {
            chapterTestId: quizId,
          },
        })
                  ...
                  return {
                    ...
                    chapters: semester.chapters.map((chapter) => {
                      if (chapter.id === chapId) {
                        return {
                          ...chapter,
                          chapterTest: chapter.chapterTest.filter(
                            (chapterTest) => chapterTest.id !== quizId
                          ),
                        };
                      } else {
                        return chapter;
  ...
  function CustomToggle({ eventKey, type }) {
    ...
    return (
      <button
        ...
        onClick={() => {
          decoratedOnClick();
          handleDropdownClick(eventKey, type);
        }}
      >
  ...
  function handleSemesterClick(semId, type) {
    ...
    if (!mainCourseData) {
      ...
      setHighlight(`semester${semId}`);
      handleSlectedIds(semId, null, null, null, "semesters");
    } else if (semId !== mainCourseData?.semesters[0].id && !isDataSaved) {
      //semid is diff and data is not saved
      //if parent data is present and user clicks on another sem and the data is not saved prompt to save

      setShowSaveDataModal(true);
    } else if (semId !== mainCourseData?.semesters[0].id && isDataSaved) {
      //semid is diff and data is saved
      ...
      <div style={{ overflow: "auto", height: "90%" }}>
        {
          <>
            <MyModal
              ...
              addSectionLevelQuiz={addSectionLevelQuiz}
              editChapterLevelQuizName={editChapterLevelQuizName}
              deleteSectionLevelQuiz={deleteSectionLevelQuiz}
              addQuizBelowChapters={addQuizBelowChapters}
              editQuizName={editQuizName}
              deleteChapterLevelQuiz={deleteChapterLevelQuiz}
            ></MyModal>
          </>
        }
        {sidebarData && (
          <Accordion defaultActiveKey={semesterDropdown}>
            {sidebarData.semesters?.map((semester, semIndex) => (
              <Card>
                <Card.Header
                  className={
                    highlight === `semester${semester.id}` ? "highlight" : ""
                  }
                >
                  <div
                    style={{
                  ...
                  >
                    <label
                      onClick={() => {
                        handleSemesterClick(semester.id, "semester");
                        setSemesterDropdown(`${semIndex}`);
                      }}
                      style={{
                        cursor: "pointer",
                        maxWidth: "70%",
                        ...
                        <Card>
                          ...
                          >
                            <div
                              ...
                              onMouseEnter={() => {
                                setIsHovering(`chapter${chapter.id}`);
                              }}
                              onMouseLeave={() => {
                                setIsHovering(null);
                              }}
                            ...
                            >
                              <label
                                onClick={() => {
                                  handleChapterClick(semester.id, chapter.id);
                                }}
                                style={{
                                  cursor: "pointer",
                                  maxWidth: "50%",
                                ...
                                <Card>
                                  ...
                                  >
                                    <div
                                      ...
                                      onMouseEnter={() => {
                                        setIsHovering(`section${section.id}`);
                                      }}
                                      onMouseLeave={() => {
                                        setIsHovering(null);
                                      }}
                                    ...
                                    >
                                      ...
                                      <span>
                                        {isHovering ===
                                          `section${section.id}` && (
                                          <i
                                            className="fa-solid fa-pen me-0"
                                            onClick={() =>
                                              setShowModal((showModal) => {
                                                return {
                                                  ...showModal,
                                        ...
                                        {isHovering ===
                                          `section${section.id}` && (
                                          <i
                                            className="fa-regular fa-circle-xmark me-0"
                                            onClick={() =>
                                              setShowModal((showModal) => {
                                                return {
                                                  ...showModal,
                                ...
                                <Card>
                                  ...
                                  >
                                    <div
                                      ...
                                      onMouseEnter={() => {
                                        setIsHovering(`chapterTest${q.id}`);
                                      }}
                                      onMouseLeave={() => {
                                        setIsHovering(null);
                                      }}
                                    ...
                                    >
                                      ...
                                      <label
                                        onClick={() => {
                                          handleChapterTestClick(
                                            semester.id,
                                            chapter.id,
                                            q.id
                                          );
                                          setCurrentSection(q.id);
                                        }}
                                        style={{
                                          cursor: "pointer",
                                          maxWidth: "50%",
                      ...
                      <Card>
                        ...
                        >
                          <div
                            ...
                            onMouseEnter={() => {
                              setIsHovering(`semesterTest${q.id}`);
                            }}
                            onMouseLeave={() => {
                              setIsHovering(null);
                            }}
                          ...
                          >
                            ...
                            <label
                              onClick={() => {
                                handleSemesterTestClick(semester.id, q.id);
                              }}
                              style={{
                                cursor: "pointer",
                                maxWidth: "50%",
        ...
        <div className="add-semester-button-container">
          ...
          >
            ...
            <i
              ...
              className="fa-solid fa-plus bg-glow"
            ></i>
          </div>
        </div>
      </div>
